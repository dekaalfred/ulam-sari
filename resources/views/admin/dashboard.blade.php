...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* SIDEBAR G�� stays dark for contrast */
aside {
...
.brand { display: flex; align-items: center; gap: 10px; padding: 20px 20px 16px; border-bottom: 1px solid var(--color-border); }
.brand-logo {
...
.brand-name { font-family: 'EB Garamond', serif; font-size: 16px; font-weight: 700; color: var(--color-primary); letter-spacing: 0.3px; }
...
.user-name { font-size: 13px; font-weight: 600; color: var(--color-primary); }
.user-role { font-size: 11px; color: var(--color-muted); margin-top: 1px; }
...
.nav-item {
  ...
  transition: all 150ms ease; border-top: none; border-bottom: none; border-right: none;
  background: none; width: 100%; text-align: left;
}
.nav-item svg { width: 16px; height: 16px; flex-shrink: 0; }
.nav-item:hover { color: var(--color-primary); background: rgba(62,39,35,0.04); }
.nav-item.active {
...
.sidebar-bottom { padding: 14px 16px; border-top: 1px solid var(--color-border); }
.btn-new {
...
.nav-footer .nav-item { border-left: none !important; }
.nav-logout { color: var(--color-red) !important; }
.nav-logout:hover { background: rgba(198,40,40,0.08) !important; }
    /* MAIN */
    header {
...
.header-title { font-family: 'EB Garamond', serif; font-size: 18px; font-weight: 700; color: var(--color-primary); }
...
<!-- ======= LOGIN ======= -->
<div id="page-login" class="{{ Auth::check() ? 'hidden' : '' }}">  <div class="login-wrap">
...
<button class="btn-login" onclick="doLogin()" type="button">
    Masuk ke Dashboard
...
<!-- ======= APP ======= -->
<div id="page-app" class="{{ Auth::check() ? '' : 'hidden' }}"><div id="app">
  <!-- SIDEBAR -->
  <aside>
    <div class="brand">
  ...
  <div style="flex:1;display:flex;flex-direction:column;min-width:0;">
    <header>
<div class="header-title">Ulam Sari Dashboard</div>
...
<div id="content-dashboard">
    ...
    <div class="page-sub">
        Welcome back. Here is the summary of Ulam Sari's operations today.
    </div>
            ...
            <div class="stat-icon stat-icon-muted">
                ...
                >
                    <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.25 0 015.25 0z"
                    />
                </svg>
...
<div id="content-menu" class="hidden">
    ...
    <div class="menu-top">
        ...
        <div>
            ...
            >
                Kelola hidangan, harga, dan ketersediaan menu restoran.
            </div>
...
<select class="modal-input" id="modal-cat">
  <option>Aneka Ikan</option><option>Aneka Ayam</option><option>Sayuran & Tumisan</option><option>Menu Lainnya</option>
</select>      </div>
      <div class="modal-field">
...
<script>
// CSRF Token Setup G�� must be 'let' because login regenerates the session/token
...
const CATS = ['Semua','Aneka Ikan','Aneka Ayam','Sayuran & Tumisan','Menu Lainnya'];
...
function renderActivity() {
  ...
  // Kalau database activity masih kosong,
  // tampilkan contoh sementara supaya desain tidak kosong.
  const displayActivities = activities.length
    ? activities
    : [
        {
          dot: 'dot-orange',
          text: 'New Reservation for 4 pax by Budi Santoso',
          sub: 'Today, 19:00 PM - Meja VIP 2',
          time_label: '10 mins ago'
        },
        {
          dot: 'dot-green',
          text: 'Menu Updated: Gurame Bakar Madu price adjusted.',
          sub: 'Updated by Admin',
          time_label: '1 hour ago'
        },
        {
          dot: 'dot-muted',
          text: 'Order Completed: Order #US-8821 finalized.',
          sub: 'Total: Rp 450.000',
          time_label: '2 hours ago'
        }
      ];

  activityList.innerHTML = displayActivities.map(a => `
    <div class="activity-item">

      <div class="activity-left">

        <div class="activity-dot ${a.dot || 'dot-muted'}"></div>

        <div class="activity-content">

          <div class="activity-text">
            ${a.text || 'Aktivitas baru'}
          </div>
...
<td>
  <button onclick="toggleMenuStatus(${m.id})" class="${m.status==='tersedia'?'status-ok':'status-bad'}" style="display:flex;align-items:center;font-size:12px;font-weight:500;background:none;border:none;cursor:pointer;padding:0;">
...
document.getElementById('menu-info').textContent = total > 0
  ? `Showing ${(menuPage - 1) * PER_PAGE + 1}-${Math.min(menuPage * PER_PAGE, total)} of ${total}`
  : '0 items';
...
pg.appendChild(
  pgBtn('‹', menuPage - 1, false, menuPage === 1)
);
...
for (let i = 1; i <= pages; i++) {
  ...
  if (
    i === 1 ||
    i === pages ||
    Math.abs(i - menuPage) <= 1
  ) {

    pg.appendChild(
      pgBtn(i, i, i === menuPage, false)
    );

  } else if (Math.abs(i - menuPage) === 2) {
    ...
    s.style.cssText =
      'color:var(--color-muted);padding:0 4px;font-size:12px;align-self:center;';
...
pg.appendChild(
  pgBtn('›', menuPage + 1, false, menuPage === pages)
);

} // penutup renderMenuTable()


function toggleAllMenu(m) {
  document
    .querySelectorAll('.menu-cb')
    .forEach(c => c.checked = m.checked);
}
...
async function toggleMenuStatus(id) {
  try {
    const res = await fetch(`/admin/api/menus/${id}/toggle`, {
      method: 'PATCH',
...
function closeRsvModal() {
    document.getElementById('modal-rsv').classList.remove('open');
}
...
document.getElementById('modal-rsv').addEventListener('click', e => {
